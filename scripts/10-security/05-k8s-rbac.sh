...
# 错误处理
trap 'log ERROR "K8s RBAC脚本异常退出 (行号: $LINENO)"' ERR
...
# Create Namespaces
# 功能: 创建标准命名空间
# 命名空间: production, staging, development, monitoring, logging, security
#-------------------------------------------------------------------------------
create_namespaces() {
...
#-------------------------------------------------------------------------------
# Create Developer RBAC
# 功能: 创建Developer角色和RoleBinding
# 特权: development/staging完整权限，production只读
# 同时创建DevOps和Monitoring角色
#-------------------------------------------------------------------------------
...
# Create LimitRanges
# 功能: 为各环境创建容器资源限制范围
# 限制: default/defaultRequest/max/min
#-------------------------------------------------------------------------------
create_limit_ranges() {
...
#-------------------------------------------------------------------------------
# Verify RBAC Configuration
# 功能: 验证RBAC配置，统计ClusterRole、RoleBinding、ServiceAccount数量
#-------------------------------------------------------------------------------
verify_rbac() {
